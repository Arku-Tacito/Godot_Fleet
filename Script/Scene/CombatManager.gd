...
func _on_Trigger_firebullet(bullet_obj, position, rotation, target, owner):
	var bu = bullet_obj.instantiate()
	bu.do_ready(position, rotation, target, owner)
	bu.connect("explode", Callable(self, "_on_explosion_effect"))	# 连接爆炸信号
	add_child(bu)

# 处理模块
func _deal_with_module(module):
	module.connect("explode", Callable(self, "_on_explosion_effect"))	# 连接爆炸信号
	if "weapon_battery" in module.get_groups():
		module.connect("firebullet", Callable(self, "_on_Trigger_firebullet"))	# 连接所有子弹发射信号

# 处理单位
func _deal_with_unit(unit):
	unit.connect("explode", Callable(self, "_on_explosion_effect"))	# 连接爆炸信号
